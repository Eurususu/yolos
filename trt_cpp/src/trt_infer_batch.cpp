...
struct Args {
    std::string model = "../weights/yolov7-tiny.engine";
    std::string source = "../data/";
    int opt_batch_size = 16;
...
class YoloTRTRunner {
    ...
    public:
        YoloTRTRunner(const std::string& engine_path, int max_batch = 32, int opt_batch = 16, 
            int max_det = 300, float conf = 0.25f, float iou = 0.7f, int cls = 80)
            : conf_thres(conf), iou_thres(iou), num_classes(cls), max_batch_size(max_batch), max_det(max_det){
...
int main(int argc, char** argv){
    ...
    for (int i = 1; i < argc; i++){
        ...
        if (arg == "--engine" && i + 1 < argc) args.model = argv[++i];
        ...
        else if (arg == "--efficient_end2end" && i + 1 < argc) args.efficient_end2end = true;
    ...
    try {
        YoloTRTRunner runner(args.model, args.max_batch_size, args.opt_batch_size, 300, args.conf_thres, args.iou_thres, args.num_classes);
        runner.run(args);
